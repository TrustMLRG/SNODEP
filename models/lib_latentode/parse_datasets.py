...
def parse_datasets(args, device):
	...
	#####################################################################
	# kinetic learning
	# Kinetic_learning dataset
	if dataset_name == "kinetic":
		# dataset_obj = HopperPhysics(root='data', download=False, generate=True, device = device)
		# dataset = dataset_obj.get_dataset()[:args.n]
		dataset_obj = None
		raw_df= pd.read_csv('data/kinetic_learning/Fulld10000n1.csv') # need to run the script from latent_ode
		time_steps= torch.tensor(raw_df['Time'], dtype= torch.float)

		arr_= []
		for x in raw_df['Strain'].unique():
			temp_df= raw_df[raw_df['Strain']==x].drop(['Strain', 'Time'], axis=1)
			arr_.append(temp_df.values)
		new_arr_= np.stack(arr_, axis=0 )
		dataset= torch.tensor(new_arr_, dtype=torch.float)
		dataset = dataset.to(device)
